...
class Film:
    '''文件类'''
    def CreateFilm(path:str,name:str,content):
        '''
        检测文件存在，不存在则创建此文件
        path:文件路径
        name: 文件名称
        ...
        if os.path.exists(f"{path}/{name}"):    
            ...
            with open(f"{path}/{name}","w+"):
                Film.WriteFilm(path , name, content)    # 顺便写入 Config.json 数据
                # try:
                Film.ReadFilm(path, name)     # 顺便读取 Config.json 数据
                # except:
                #     time.sleep(5)
...
class DateDeal:
    ...
    def Banner(e):
        ...
        print(BannerForm)
        print(e.getName())
        Film.CreateFilm(ConfigPath,"banner.json",BannerForm)
        if e.getName() in Banner:
            return False
        else:
            return True
class onListener:
    ...
    def onCmd(e , cmd):
        ...
        '''
        if cmd == 'calla':
            if DateDeal.Banner(e) :
                GUI.AllGUI(e)
                return False
            else:
               e.sendText("§l§7[§2Call§7]§c你已被列为黑名单！") 
        elif cmd == "callp":
            if DateDeal.Banner(e) :    
                GUI.PersonGUI(e)
                return False
...
Film.CreateFilm(ConfigPath,"config.json",ConfigForm)
# 一大堆监听器和注册指令
